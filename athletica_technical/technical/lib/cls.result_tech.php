...
function getNofResults($event, $xSerienstart = 0, $maxRang = 0, $details = true) {
    ...
    try {
        ...
        if($eval != $cfgEvalType[$strEvalTypeHeat]) {   
            ...
            if($final['Endkampf'] == 1){
                ...
                $attempt = getCurrentAttempt($event, $final['Finalisten']);
...
function checkDrop($event, $maxRang) {
    ...
    try {      
        ...
        $query_attempts->bindValue(':serie', $event);
        if($maxRang > 0) {
