...
/*
 *	set groups for team sm event
 *
 **************************************************************************************/
$page->printPageTitle($strCombinedGroupsAutoAssign.", ".$strEventTypeTeamSM);
...
<table><tr>
	...
	<td class='forms'>
		<form name="discipline" action="meeting_entries_setgroups.php" method="GET">
		<input type="hidden" name="category" value="<?php echo $category ?>">
		<?php
		
		if($category > 0){
			$sqlCategory = "AND w.xKategorie = $category";
		}
		
		$dropdown = new GUI_Select("disc", 1, "document.discipline.submit()");
...
<?php
//
// show entries for discipline
//
if($disc > 0){
?>
<table class='dialog'>
	<tr>
		<form action="meeting_entries_setgroups.php?sort=<?php echo $_GET['sort'] ?>&category=<?php echo $category ?>&disc=<?php echo $disc ?>"
			method="POST" name="groups">
		<input type="hidden" name="arg" value="teamsm_assign">
		<th class='dialog'><?php echo $strAutomatic ?>:</th>
		<td class='forms'> max <input type="text" name='max' value="10" size="2"> <?php echo $strAthletes ?></td>
		<td class='forms'><input type="submit" value="<?php echo $strAssign ?>"></td>
		<td class='dialog'><?php echo $strCombinedGroupsBySort ?></td>
	...
	<tr>
		<td class='forms' colspan='2'>
			<input type="button" value="<?php echo $strRemoveGroups ?>"
				onclick="document.groups.arg.value = 'teamsm_remove'; document.groups.submit()">
		</td>
	</tr>
</table>
<br>
<table class='dialog'>
	<tr>
		<th class='dialog'><a href='meeting_entries_setgroups.php?sort=nbr&category=<?php echo $category ?>&disc=<?php echo $disc ?>'>
			<?php echo $strStartnumber ?><img src="<?php echo $img_nbr?>"></a></th>
		<th class='dialog'><a href='meeting_entries_setgroups.php?sort=name&category=<?php echo $category ?>&disc=<?php echo $disc ?>'>
			<?php echo $strName ?><img src="<?php echo $img_name?>"></a></th>
		<th class='dialog'><a href='meeting_entries_setgroups.php?sort=pole&category=<?php echo $category ?>&disc=<?php echo $disc ?>'>
			<?php echo $strPole ?><img src="<?php echo $img_pole?>"></a></th>
		<th class='dialog'><a href='meeting_entries_setgroups.php?sort=high&category=<?php echo $category ?>&disc=<?php echo $disc ?>'>
			<?php echo $strHigh ?><img src="<?php echo $img_high?>"></a></th>
		<!--<th class='dialog'><a href='meeting_entries_setgroups.php?sort=comb&category=<?php echo $category ?>&disc=<?php echo $disc ?>'>
			<?php echo $strCombinedShort ?><img src="<?php echo $img_comb?>"></a></th>-->
		<th class='dialog'><?php echo $strGroup ?></th>
	</tr>
		<form action="meeting_entries_setgroups.php?sort=<?php echo $_GET['sort'] ?>&category=<?php echo $category ?>&disc=<?php echo $disc ?>" method="POST">
	...
	if(mysql_errno() > 0){
		...
		foreach($current as $curr){
			...
			$i++;
			if($_POST['arg'] == "teamsm_assign"){ // if auto assign is choosen, set group for entry
				$curr[3] = $group;
				mysql_query("UPDATE anmeldung SET Gruppe = $group WHERE xAnmeldung = ".$curr[0]);
			}elseif($_POST['arg'] == "teamsm_remove"){
				$curr[3] = "";
